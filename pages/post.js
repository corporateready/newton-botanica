      ...
      <div className={styles.container}>
        ...
        {isToggleLang === "ro" ? (
          <h4 className={styles.post__title}>
            ...
            <br />
            electrică la <span>NEWTON HOUSE Grădina Botanică:</span>
          </h4>
        ) : (
          <h4 className={styles.post__title}>
        ...
        {isToggleLang === "ro" ? (
          <p className={styles.post__subtext}>
            Oare cum se va schimba lumea și transportul urban în anii ce
            urmează? Probabil ai auzit că începând cu anul 2035, Europa va
            interzice producția mașinilor pe benzină și diesel? Echipa{" "}
            <span>NEWTON HOUSE</span> a acționat proactiv, integrând în
            complexul de la Botanica stații de încărcare electrice. Acestea
            permit locuitorilor ce procură avantajos, în rate pe 5 ani, un loc
            de parcare cu priză electrică — să încarce orice timp de transport
            electric. 
          </p>
        ) : (
          <p className={styles.post__subtext}>
            Как изменится мир и городской транспорт в ближайшие годы? Наверняка,
            ты знаешь, что с 2035 года в Европе будет запрещено производство
            бензиновых и дизельных автомобилей? Команда{" "}
            <span>NEWTON HOUSE</span> предприняла активные действия,
            интегрировав в жилой комплекс на Ботанике электрозарядные станции.
            Они позволяют жителям, купившим парковочное место с розеткой,
            заряжать электротранспорт в любое время.
          </p>
        )}
        {isToggleLang === "ro" ? (
          <p className={styles.post__text}>
            Mașinile pe benzină și diesel sunt pe cale să devină rarități odată
            ce Uniunea Europeană a propus ca, începând cu anul 2035, să fie
            interzisă producția lor. Multă lume evită încă mașinile electrice
            din considerentul lipsei stațiilor de încărcare. Mai cu seamă, pe
            timp de noapte este o adevărată problemă. Cu toate că reticența e
            încă mare, viitorul aparține mașinilor electrice. Iar disconfortul
            și timpul de așteptare pentru încărcarea lor pot fi rezolvate ușor
            dacă deții un loc de parcare propriu, dotat cu stație de încărcare
            specială.
          </p>
        ) : (
          <p className={styles.post__text}>
            Автомобили с двигателями внутреннего сгорания в ближайшие годы
            станут раритетом, поскольку Европейский союз предложил запретить их
            производство с 2035 года. Однако несмотря на привлекательность
            электромобилей, многие люди продолжают избегать их покупки из-за
            отсутствия широкой сети зарядных станций. В особенности, эта
            проблема становится острой в ночное время. И хотя сомнения населения
            все еще велики, будущее за электромобилями. А о неудобствах и
            времени ожидания зарядки можно легко забыть, если есть собственное
            парковочное место со специальной зарядной станцией.
          </p>
        )}
        ...
        {isToggleLang === "ro" ? (
          <p className={styles.post__text}>
            Fiecare doritor de a contribui la un mediu ecologic, de a face
            economii la costurile de întreținere a mașinii sau, pur și simplu,
            care este în pas cu schimbările și inovațiile globale — are
            oportunitatea să cumpere un loc de parcare echipat cu priză
            electrică în complexul rezidențial{" "}
            <span>NEWTON HOUSE Grădina Botanică</span> și să beneficieze de
            facilități multiple: 
          </p>
        ) : (
          <p className={styles.post__text}>
            Каждый, кто хочет внести свой вклад в сохранение окружающей среды,
            сэкономить на содержании автомобиля или просто двигаться в ногу с
            глобальными изменениями и инновациями, имеет возможность приобрести
            парковочное место, оборудованное электрической розеткой, в жилом
            комплексе <span>NEWTON HOUSE Grădina Botanică</span> и
            воспользоваться многочисленными удобствами:
          </p>
        )}
        <div className={styles.post__list_block}>
          <div className={styles.post__list_icon}>
            <svg
              width="100%"
              height="100%"
              viewBox="0 0 18 23"
            ...
            >
              <path
                d="M0 5.69865V23H9H18V5.69865L8.85305 0L0 5.69865Z"
                fill="#2BB018"
              />
            </svg>
          </div>
          ...
          {isToggleLang === "ro" ? (
            <p className={styles.post__list_text}>
              <span>Confort și securitate.</span> Proprietarii de vehicule
              electrice au ocazia să cumpere un un loc de parcare subteran dotat
              cu priză electrică și să încarce mașinile pe timp de noapte,
              beneficiind de confort și securitate suplimentară. Prizele sunt
              protejate de o cutie metalică și un lacăt pentru a preveni
              utilizarea neautorizată. De asemenea, fiecare priză este conectată
              la un contor pentru a monitoriza cantitatea de energie electrică
              consumată. Iar în cazuri de urgențe, conectarea unui redresor e
              simplă și rapidă.
            </p>
          ) : (
            <p className={styles.post__list_text}>
              <span>Комфорт и безопасность. </span>Владельцы электромобилей
              могут купить парковочное место с электрической розеткой и заряжать
              свои автомобили, пользуясь дополнительным комфортом и
              безопасностью. Розетки защищены металлическим коробом и замком,
              что предотвращает их несанкционированное использование. Каждая
              розетка также подключена к индивидуальному счетчику, позволяющему
              контролировать количество потребляемой электроэнергии. 
            </p>
          )}
        </div>
        <div className={styles.post__list_block}>
          ...
          {isToggleLang === "ro" ? (
            <p className={styles.post__list_text}>
              ...
              aspiratoarelor auto și a stabilizatoarelor de tensiune, pentru a
              alimenta acumulatorul.
            </p>
          ) : (
            <p className={styles.post__list_text}>
              <span>Множество вариантов использования. </span>Электрические
              розетки универсальны и могут использоваться для самых разных
              целей: от зарядки электромобилей и плагин-гибридов,
              электровелосипедов и скутеров до подключения автомобильных
              пылесосов и выпрямителя напряжения, чтобы «прикурить» аккумулятор.
            </p>
          )}
        </div>
        <div className={styles.post__list_block}>
          ...
          {isToggleLang === "ro" ? (
            ...
            <p className={styles.post__list_text}>
              <span>Возможность совместного использования. </span>Существует
              возможность разделить с соседом как парковочное место с
              электрической розеткой, так и расходы на его приобретение.
            </p>
          )}
        </div>
        <div className={styles.post__list_block}>
          ...
          {isToggleLang === "ro" ? (
            <p className={styles.post__list_text}>
              <span>Susținerea mobilității ecologice. </span>
              Un pas concret către contribuirea la un mediu mai ecologic și
              susținerea tranziției către mobilitatea electrică.
            </p>
          ) : (
            <p className={styles.post__list_text}>
              <span>«Зеленые» технологии. </span>Решительный шаг в сторону
              улучшения состояния экологии окружающей среды и поддержка перехода
              на электромобили.
            </p>
          )}
        </div>
        <div className={styles.post__list_block}>
          ...
          {isToggleLang === "ro" ? (
            <p className={styles.post__list_text}>
              <span>Metodă comodă de plată. </span>
              Dezvoltatorul oferă o metodă extrem de avantajoasă, ce permite
              achitarea în rate pe o durată de 5 ani cu plata trimestrială, fără
              comisioane și gajuri.
            </p>
          ) : (
            <p className={styles.post__list_text}>
              <span>Удобный способ оплаты. </span>Застройщик предлагает
              чрезвычайно выгодную возможность рассрочки для парковочных мест:
              выплачивая сумму ежеквартальными платежами в течение 5 лет без
              комиссий и залога. (Розетка может быть куплена в рассрочку только
              на год).
            </p>
          )}
        </div>
        <div className={styles.post__list_block}>
          ...
          {isToggleLang === "ro" ? (
            <p className={styles.post__list_text}>
              <span>Investiție strategică. </span>
              Acumularea unui avantaj competitiv prin investirea într-un loc de
              parcare echipat cu priză electrică, anticipând necesitățile
              viitoare în contextul reglementărilor europene privind vehiculele
              pe benzină sau diesel.
            </p>
          ) : (
            <p className={styles.post__list_text}>
              <span>Стратегическая инвестиция. </span>Позаботься о будущем,
              инвестировав в парковочное место, оборудованное электрической
              розеткой, опережая возможные неудобства, связанные с европейскими
              ограничениями по бензиновым и дизельным автомобилям.
            </p>
          )}
        </div>
        {isToggleLang === "ro" ? (
          <p className={styles.post__text}>
            <i>
              Conform statisticilor, se estimează că, până în 2030, aproximativ
              31% dintre mașinile vândute vor fi electrice. Conform Agenției
              Internaționale pentru Energie (IEA), parcările rezidențiale dotate
              cu puncte de încărcare vor înregistra o creștere de 40%
            </i>{" "}
            în aceeași perioadă. Iar pentru că{" "}
            <span>NEWTON HOUSE Grădina Botanică</span> este mereu în pas cu
            tendințele globale — facilitează accesul la stații de încărcare
            electrice, oferind locuitorilor din complex posibilitatea de a
            procura locuri de parcare cu prize electrice în parcarea subterană. 
          </p>
        ) : (
          <p className={styles.post__text}>
            <i>
              Согласно прогнозам, к 2035 году более 30% продаваемых автомобилей
              будут электрическими. По данным Международного энергетического
              агентства (МЭА), за этот же период количество парковок с пунктами
              зарядки в жилых комплексах
            </i>{" "}
            увеличится на 40%. А поскольку{" "}
            <span>NEWTON HOUSE Grădina Botanică </span>
            стремится идти в ногу с мировыми тенденциями, застройщик упрощает
            доступ к новым технологиям и предоставляет жителям комплекса
            возможность приобрести парковочные места на подземной стоянке,
            оборудованные электрическими розетками. 
          </p>
        )}
        {isToggleLang === "ro" ? (
          <p className={styles.post__text}>
            Amintim că <span>NEWTON HOUSE Grădina Botanică</span> este unicul
            complex din Chișinău ce îmbină zona de parc și o infrastructură
            internă dezvoltată, cu toate facilitățile la distanță de câteva
            minute: parcare subterană multietajată, grădiniță, sală de fitness
            cu piscină acoperită, restaurant, centru comercial propriu, alee de
            promenadă, piste pentru biciclete, terenuri de joacă pentru 5
            categorii de vârste, camere „baby care”, zone de relaxare imense,
            terenuri de sport fără precedent în țară, depozit pentru
            echipamentul sezonier și chiar o zonă amenajată pentru animalele de
            companie.  
          </p>
        ) : (
          <p className={styles.post__text}>
            Напомним, <span>NEWTON HOUSE Grădina Botanică</span> — это
            единственный комплекс в Кишиневе, сочетающий расположение напротив
            ботанического сада и развитую внутреннюю инфраструктуру, при этом
            каждый из объектов находится в нескольких минутах ходьбы. Жильцам
            комплекса доступны многоэтажная подземная стоянка, детский сад,
            фитнес-зал с крытым бассейном, ресторан, торговый центр, прогулочная
            аллея, велосипедные дорожки, игровые площадки для 5 возрастных
            групп, санитарные комнаты «baby-care», огромные зоны отдыха,
            беспрецедентные для Молдовы спортивные площадки, хранилище для
            сезонного инвентаря и даже зона для выгула домашних животных.
          </p>
        )}
        ...
        <p className={styles.post__text_bottom}>
